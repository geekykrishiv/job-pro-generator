...
export async function generateTailoredResume({
  ...
  // Step 3: Call Gemini
  const raw = await callGemini(apiKey, prompt, TAILORING_SYSTEM_PROMPT);
  ...
  if (!validation.valid) {
    ...
    const fixRaw = await callGemini(apiKey, fixPrompt, "Fix the LaTeX code. Return ONLY valid LaTeX starting with \\documentclass and ending with \\end{document}.");
...
export async function refineTailoredResume(
  ...
  const raw = await callGemini(apiKey, prompt, REFINE_SYSTEM_PROMPT);
